...
const Dropzone = ({setUploadedFiles}) => {
    ...
    return <>
        <div ref={dropzoneRef}
             className="w-1/2 h-24 bg-gray-100 rounded-lg mt-4 flex items-center justify-center space-x-4 border border-2 border-dashed hover:border-green-700 transition">
            <BsUpload/>
            <p>파일 업로드</p>
        </div>
        <input ref={fileInputRef} style={{display: 'none'}} multiple={true} type={'file'}
               onChange={(e) => {
